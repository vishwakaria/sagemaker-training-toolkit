...
@patch("asyncio.create_subprocess_shell")
@patch("sagemaker_training.environment.Environment")
def test_smdataparallel_run_accl_default_on_and_installed_python(
    training_env,
    async_shell,
    policy,
    ssh_client,
    ...
    with patch.dict(os.environ, clear=True):
        ...
        async_shell.assert_called_once()
        assert process == async_shell.return_value
        path_exists.assert_called_with("/opt/conda/lib/libacclcoll.so", 1)
...
@patch("asyncio.create_subprocess_shell")
@patch("sagemaker_training.environment.Environment")
def test_smdataparallel_run_accl_enabled_and_not_installed_python(
    training_env,
    async_shell,
    policy,
    ssh_client,
    ...
    with patch.dict(os.environ, clear=True):
        ...
        with pytest.raises(ValueError):
            smdataparallel_runner.run(wait=False)
